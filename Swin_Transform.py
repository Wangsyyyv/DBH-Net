...
class SwinTransformerSys(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1,
        ...
        self.rfb1 = RFB_modified(192, 48)
        self.rfb2 = RFB_modified(384, 48)
        self.rfb3 = RFB_modified(768, 48)
    ...
    def forward_features(self, x):
        ...
        x3 = self.norm3(x)  # [B, L, C]
        x3 = x3.transpose(1, 2)
        x3 = x3.reshape(x3.shape[0], x3.shape[1], H, W)

        x2 = self.norm2(x_downsample[2])
        x2 = x2.transpose(1, 2)
        x2 = x2.reshape(x2.shape[0], x2.shape[1], H * 2, W * 2)

        x1 = self.norm1(x_downsample[1])
        x1 = x1.transpose(1, 2)
        x1 = x1.reshape(x1.shape[0], x1.shape[1], H * 4, W * 4)

        x0 = self.norm0(x_downsample[0])
        x0 = x0.transpose(1, 2)
        x0 = x0.reshape(x0.shape[0], x0.shape[1], H * 8, W * 8)
    ...
    def forward(self, x):
        x3, x2, x1, x0, x_s_feat = self.forward_features(x)
        return x3, x2, x1, x0, x_s_feat
...
if __name__ == '__main__':
    ...
    # img3, img2, img1, img0, img = model(img)
    # # img = model(img)
    # # for i in img:
    # #     print(i.shape)
    # print(img3.shape, img2.shape, img1.shape, img0.shape, img.shape)
