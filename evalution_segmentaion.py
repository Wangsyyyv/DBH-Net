...
def calc_semantic_segmentation_confusion(pred_labels, gt_labels,n_class):
    ...
    Args:
        pred_labels (iterable of numpy.ndarray): A collection of predicted
            labels. The shape of a train_label array
            is :math:`(H, W)`. :math:`H` and :math:`W`
            are height and width of the train_label.
        gt_labels (iterable of numpy.ndarray): A collection of ground
            truth labels. The shape of a ground truth train_label array is
            :math:`(H, W)`, and its corresponding prediction train_label should
            have the same shape.
    ...
    for pred_label, gt_label in zip(pred_labels, gt_labels):
        ...
        confusion += np.bincount(
            n_class * gt_label[mask].astype(int) + np.array(pred_label[mask],dtype=np.int64),
...
def calc_semantic_segmentation_iou(confusion):
    ...
    * :math:`\\text{IoU of the i-th class} =  \
    ...
    iou1 = confusion[1][1] / (confusion[1][0] + confusion[1][1] + confusion[0][1])
    return iou
    # return iou


def eval_semantic_segmentation(pred_labels, gt_labels,n_classes):
    ...
    * :math:`\\text{Mean Class Accuracy} = \\frac{1}{k} \
    ...
    confusion = calc_semantic_segmentation_confusion(
        pred_labels, gt_labels,n_classes)
